...
func ProxyFactory(config *ProxyConfig) (Proxy, error) {
	...
	case SOCKSProxy:
		// SOCKS proxy will be implemented in socks.go
		return nil, fmt.Errorf("%w: SOCKS proxy not implemented yet", ErrProxyNotSupported)
	case HTTPProxy:
		// HTTP proxy will be implemented in http.go
		return nil, fmt.Errorf("%w: HTTP proxy not implemented yet", ErrProxyNotSupported)
	default:
