...
func TestProxyFactory(t *testing.T) {
	...
	proxy, err = ProxyFactory(config)
	if err != nil {
		t.Errorf("ProxyFactory failed with SOCKS proxy: %v", err)
	}
	if proxy == nil {
		t.Error("ProxyFactory returned nil proxy for SOCKS proxy")
	}
	...
	proxy, err = ProxyFactory(config)
	if err != nil {
		t.Errorf("ProxyFactory failed with HTTP proxy: %v", err)
	}
	if proxy == nil {
		t.Error("ProxyFactory returned nil proxy for HTTP proxy")
	}
}
