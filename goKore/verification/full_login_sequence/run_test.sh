...
run_full_login_sequence_test() {
    ...
    log "Building full login sequence test binary..."
    
    # Change to the test directory
    pushd "$BASE_DIR" > /dev/null
    
    # Build the test binary
    go build -o "full_login_sequence_test" .
    
    if [ $? -ne 0 ]; then
        log_error "Failed to build full login sequence test binary"
        popd > /dev/null
        return 1
    fi
    
    log_success "Successfully built full login sequence test binary"
    
    # Run the test
    log "Running full login sequence test..."
    
    ./full_login_sequence_test > "$BASE_DIR/results/full_login_sequence_result.txt" 2> "$BASE_DIR/results/full_login_sequence_error.txt"
